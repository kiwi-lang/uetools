...
from gamekit.conf import Command, get_build_platforms, load_conf, ubt, guess_platform, get_build_modes, LINUX
...
class Build(Command):
    ...
    .. code-block:: console
       ...
       # Builds the Edidtor for the `GamekitDev` project
       gkcli build GamekitDevEditor

       # Runs all the command UAT would execute to compile the project
       gkcli build GamekitDev --profile update-project
    ...
    def arguments(subparsers):
        ...
        build.add_argument("--platform", type=str, default=guess_platform(), help='Platform to build for', choices=set(list(get_build_platforms())))
        build.add_argument("--mode", type=str, default='Development', help='Build mode', choices=list(get_build_modes()))
        build.add_argument('--profile', type=str, default=None, help='Build multiple targets using a configuration')
    ...
    def execute(args):
        ...
        logfile = 'UBT-{}-{}-{}.txt'.format(target, platform, mode)
        ...
        #
        exec = [ubt()] + args
        print(' '.join(exec), flush=True)

        p = subprocess.run(exec,
